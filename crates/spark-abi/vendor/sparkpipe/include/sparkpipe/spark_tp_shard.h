#ifndef SPARKPIPE_SPARK_TP_SHARD_H
#define SPARKPIPE_SPARK_TP_SHARD_H
...
#define SPARK_TP_SHARD_ABI_VERSION 1u

typedef enum SparkTpShardClass
{
	SPARK_TP_SHARD_CLASS_UNKNOWN = 0,
	...
	// An [a; b] output concatenation: each half output-splits and the halves
	// re-concatenate per rank, so a rank's slice is never a plain row range.
	// The engine refuses to plain-split these; the loader's concat-aware
	// path (or an offline slicer) owns them.
	SPARK_TP_SHARD_CLASS_CONCAT_OUTPUT
} SparkTpShardClass;

typedef struct SparkTpShapeDescriptor
{
...
typedef struct SparkTpModelGeometry
{
	uint32_t abi_version;
	uint32_t head_count;
...
} SparkTpModelGeometry;

typedef struct SparkTpShardView
{
	uint32_t abi_version;
...
static inline void SparkTpModelGeometryFromModel(SparkTpModelGeometry *geometry)
{
...
// The classification is the MODEL's knowledge, injected: a table function
// from tensor name to shard class, plus the head block each head-split class
// uses. The engine owns only the arithmetic and the refusal.
uint32_t SparkTpShardNameEndsWith(const char *tensor_name, const char *suffix);

typedef SparkTpShardClass (*SparkTpShardClassifier)(const char *tensor_name);
typedef uint64_t (*SparkTpShardHeadBlock)(const char *tensor_name, const SparkTpModelGeometry *geometry);
