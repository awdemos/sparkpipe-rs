...
#define SPARK_RING_RUNTIME_HOST_NAME_BYTES 16u
#define SPARK_RING_RUNTIME_ROUTE_NAME_BYTES 64u
#define SPARK_RING_RUNTIME_PACK_PATH_BYTES 512u
#define SPARK_RING_RUNTIME_DEFAULT_PORT_BASE 52100u
#define SPARK_RING_RUNTIME_FINAL_EVENT_PORT_OFFSET 200u
#define SPARK_RING_RUNTIME_FINAL_EVENT_ROUTE_NAME_BYTES 64u
#define SPARK_RING_RUNTIME_FINAL_EVENT_MAGIC 0x35454650u
#define SPARK_RING_RUNTIME_HIDDEN_DIMENSION \
    SPARK_GLM52_MODEL_HIDDEN_DIMENSION
#define SPARK_RING_RUNTIME_DEFAULT_QUANTIZATION_MODE \
...
#define SPARK_RING_RUNTIME_INDEXSHARE_SIDEBAND_BYTES_PER_SEQUENCE \
    SPARK_GLM52_MODEL_DSA_SELECTED_INDEX_BYTES
#define SPARK_RING_RUNTIME_DSPARK_TAP_SIDEBAND_BYTES_PER_SEQUENCE \
    (SPARK_GLM52_DSPARK_AUX_LAYER_COUNT * \
     SPARK_RING_RUNTIME_BF16_HIDDEN_BYTES_PER_SEQUENCE)
#define SPARK_RING_RUNTIME_MAX_SIDEBAND_BYTES_PER_SEQUENCE \
    (SPARK_RING_RUNTIME_INDEXSHARE_SIDEBAND_BYTES_PER_SEQUENCE + \
     SPARK_RING_RUNTIME_DSPARK_TAP_SIDEBAND_BYTES_PER_SEQUENCE)
#define SPARK_RING_RUNTIME_MAX_TRANSPORT_BYTES_PER_SEQUENCE \
    (SPARK_RING_RUNTIME_BF16_HIDDEN_BYTES_PER_SEQUENCE + \
     SPARK_RING_RUNTIME_MAX_SIDEBAND_BYTES_PER_SEQUENCE)
#define SPARK_RING_RUNTIME_LAYER_MAJOR_TRANSPORT_BYTES_PER_ROW \
	(SPARK_RING_RUNTIME_BF16_HIDDEN_BYTES_PER_SEQUENCE + \
	 SPARK_RING_RUNTIME_INDEXSHARE_SIDEBAND_BYTES_PER_SEQUENCE)
#define SPARK_RING_RUNTIME_MAX_SPECULATIVE_ROWS_PER_LANE \
	SPARK_GLM52_MODEL_MAX_SPECULATIVE_ROWS_PER_LANE

#define SPARK_RING_RUNTIME_RANK_FLAG_HAS_PREVIOUS 0x00000001u
#define SPARK_RING_RUNTIME_RANK_FLAG_HAS_NEXT 0x00000002u
#define SPARK_RING_RUNTIME_RANK_FLAG_FINAL_STAGE 0x00000004u
#define SPARK_RING_RUNTIME_RANK_FLAG_DENSE_PREFIX 0x00000008u
#define SPARK_RING_RUNTIME_RANK_KNOWN_FLAGS \
    (SPARK_RING_RUNTIME_RANK_FLAG_HAS_PREVIOUS | \
     SPARK_RING_RUNTIME_RANK_FLAG_HAS_NEXT | \
     SPARK_RING_RUNTIME_RANK_FLAG_FINAL_STAGE | \
     SPARK_RING_RUNTIME_RANK_FLAG_DENSE_PREFIX)

typedef struct SparkRingRuntimeRankPlan
{
    ...
    char host_name[SPARK_RING_RUNTIME_HOST_NAME_BYTES];
    char previous_host_name[SPARK_RING_RUNTIME_HOST_NAME_BYTES];
    char next_host_name[SPARK_RING_RUNTIME_HOST_NAME_BYTES];
    char tp_peer_host_names[SPARK_TP_COLLECTIVE_MAX_STEPS][SPARK_RING_RUNTIME_HOST_NAME_BYTES];
    uint32_t tp_peer_ports[SPARK_TP_COLLECTIVE_MAX_STEPS];
    char input_route_name[SPARK_RING_RUNTIME_ROUTE_NAME_BYTES];
    char output_route_name[SPARK_RING_RUNTIME_ROUTE_NAME_BYTES];
    SparkHiddenTransportEndpoint input_endpoint;
    SparkHiddenTransportEndpoint output_endpoint;
} SparkRingRuntimeRankPlan;
...
typedef struct SparkRingRuntimeFinalEventRoute
{
    uint32_t abi_version;
    uint32_t descriptor_bytes;
    ...
    char source_host_name[SPARK_RING_RUNTIME_HOST_NAME_BYTES];
    char sink_host_name[SPARK_RING_RUNTIME_HOST_NAME_BYTES];
    char route_name[SPARK_RING_RUNTIME_FINAL_EVENT_ROUTE_NAME_BYTES];
} SparkRingRuntimeFinalEventRoute;

typedef struct SparkRingRuntimeFinalEvent
{
    uint32_t magic;
    uint32_t descriptor_bytes;
    uint32_t status;
...
#define SPARK_RING_RUNTIME_FINAL_EVENT_FLAG_DSPARK_DRAFT 0x00000001u
#define SPARK_RING_RUNTIME_FINAL_EVENT_KNOWN_FLAGS \
	SPARK_RING_RUNTIME_FINAL_EVENT_FLAG_DSPARK_DRAFT

#define SPARK_RING_RUNTIME_FINAL_EVENT_ROUTE_DESCRIPTOR_BYTES \
    ((uint32_t)sizeof(SparkRingRuntimeFinalEventRoute))
#define SPARK_RING_RUNTIME_FINAL_EVENT_DESCRIPTOR_BYTES \
    ((uint32_t)sizeof(SparkRingRuntimeFinalEvent))
