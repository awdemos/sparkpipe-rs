...
static SparkStatus SparkQwen36WorkControlValidatePlanConfiguration(
	const SparkQwen36WorkControlKvPlanConfig *configuration)
{
	if ( configuration == 0
...
static SparkStatus SparkQwen36WorkControlPrepareBlock(
	const SparkQwen36WorkControlKvPlanConfig *configuration,
...
static SparkStatus SparkQwen36WorkControlProgressBatch(
	SparkStageKvClient *client,
	SparkQwen36WorkControlKvBatchState *batch_state)
{
	SparkKvStoreCompletion completion;
	SparkStatus status;
	if ( batch_state->state != SPARK_QWEN36_WORK_CONTROL_BATCH_SUBMITTED )
		return(SPARK_STATUS_OK);
...
uint32_t SparkQwen36WorkControlGdnBlockEquivalents(
...
SparkStatus SparkQwen36WorkControlCumulativeNonresident(
	const SparkQwen36WorkControlPendingLane *pending_lanes,
	...
	uint32_t *cumulative_nonresident_block_counts)
{
	...
	for ( packet_index = 0u; packet_index < packet_count; ++packet_index )
	{
		...
		for ( packet_lane_index = 0u;
			...
			++packet_lane_index )
		{
			const SparkQwen36WorkControlPendingLane *lane;
			uint64_t lane_count;
...
uint32_t SparkQwen36WorkControlSelectRestorePackets(
	const SparkQwen36WorkControlKvPlanConfig *configuration,
	uint32_t packet_count,
...
SparkStatus SparkQwen36WorkControlBuildRestoreBatch(
	const SparkQwen36WorkControlKvPlanConfig *configuration,
	const SparkQwen36WorkControlPendingLane *pending_lanes,
	uint32_t pending_lane_count,
	const uint32_t *packet_lane_counts,
	...
	uint32_t *lanes_built)
{
	...
	SparkStatus status;
	status = SparkQwen36WorkControlValidatePlanConfiguration(configuration);
	...
	*lanes_built = 0u;
	...
	for ( lane_index = 0u; lane_index < pending_lane_count; ++lane_index )
	{
		...
		if ( lane->gdn_nonresident != 0u )
		{
			status = SparkQwen36WorkControlPrepareBlock(
...
SparkStatus SparkQwen36WorkControlSubmit(
	SparkStageKvClient *client,
	SparkQwen36WorkControlKvBatchState *batch_state,
	uint32_t operation,
	...
	uint32_t priority)
{
	...
	if ( client == 0 || batch_state == 0 || blocks == 0
		...
		|| priority > SPARK_QWEN36_WORK_CONTROL_RESTORE_PRIORITY_SPECULATIVE )
		return(SPARK_STATUS_INVALID_ARGUMENT);
	...
	batch_state->state = SPARK_QWEN36_WORK_CONTROL_BATCH_SUBMITTED;
	batch_state->submitted_block_count = block_count;
	return(SPARK_STATUS_OK);
}

SparkStatus SparkQwen36WorkControlProgress(
	SparkStageKvClient *client,
	SparkQwen36WorkControlKvState *state)
{
	SparkStatus status;
	if ( client == 0 || state == 0 )
		return(SPARK_STATUS_INVALID_ARGUMENT);
	status = SparkQwen36WorkControlProgressBatch(client,&state->restore);
	if ( status != SPARK_STATUS_OK )
		return(status);
	return(SparkQwen36WorkControlProgressBatch(client,&state->evict));
}

SparkStatus SparkQwen36WorkControlAcknowledge(
	SparkQwen36WorkControlKvBatchState *batch_state)
{
	if ( batch_state == 0
		|| batch_state->state != SPARK_QWEN36_WORK_CONTROL_BATCH_READY )
