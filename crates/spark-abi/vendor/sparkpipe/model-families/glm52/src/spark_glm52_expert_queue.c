...
static uint32_t SparkGlm52ExpertQueueConfigurationIsValid(
	...
	const SparkGlm52ExpertQueueConfiguration *configuration)
{
	...
	if ( queue == 0 || !SparkGlm52ExpertQueueConfigurationIsValid(configuration) )
...
static void SparkGlm52ExpertQueueReleaseRow(
	SparkGlm52ExpertQueue *queue,
	uint32_t row_index)
{
	...
	uint64_t arrival_ns)
{
	...
	if ( queue == 0 || layer_index >= queue->layer_count ||
		expert_index >= queue->expert_count )
		return(SPARK_STATUS_INVALID_ARGUMENT);
	if ( queue->mode == SPARK_GLM52_EXPERT_QUEUE_MODE_SEALED_BATCH &&
		queue->layer_sealed[layer_index] != 0u )
	...
	uint32_t layer_index)
{
	...
	if ( queue->layer_sealed[layer_index] != 0u )
		return(SPARK_STATUS_BUSY);
	if ( queue->layer_enqueued_row_count[layer_index] == 0u )
		return(SPARK_STATUS_NOT_FOUND);
	for ( expert_index = 0u; expert_index < queue->expert_count; ++expert_index )
		if ( queue->slots[layer_index][expert_index].count >
			SPARK_GLM52_EXPERT_QUEUE_MAX_FIRING_ROWS )
			return(SPARK_STATUS_CAPACITY_EXCEEDED);
	queue->layer_sealed[layer_index] = 1u;
	return(SPARK_STATUS_OK);
}

static uint32_t SparkGlm52ExpertQueueSlotShouldFire(
	const SparkGlm52ExpertQueue *queue,
	uint32_t layer_index,
	const SparkGlm52ExpertQueueSlot *slot,
	uint64_t now_ns)
{
	if ( slot->count == 0u )
		return(0u);
	if ( queue->mode == SPARK_GLM52_EXPERT_QUEUE_MODE_SEALED_BATCH )
		return(queue->layer_sealed[layer_index]);
	...
	SparkGlm52ExpertQueueFiring *firing_out)
{
	...
	for ( layer_index = 0u; layer_index < queue->layer_count; ++layer_index )
	{
		...
		{
			slot = &queue->slots[layer_index][expert_index];
			if ( !SparkGlm52ExpertQueueSlotShouldFire(
				queue,layer_index,slot,now_ns) )
				continue;
			emit_count = queue->mode == SPARK_GLM52_EXPERT_QUEUE_MODE_SEALED_BATCH
				? slot->count
				: (slot->count < SPARK_GLM52_EXPERT_QUEUE_MAX_FIRING_ROWS
					? slot->count
					: SPARK_GLM52_EXPERT_QUEUE_MAX_FIRING_ROWS);
			...
			row_index = slot->head;
			for ( emit_index = 0u; emit_index < emit_count; ++emit_index )
			{
				uint32_t next_row_index;

				next_row_index = queue->rows[row_index].list_next;
			...
			if ( queue->mode == SPARK_GLM52_EXPERT_QUEUE_MODE_SEALED_BATCH &&
				queue->layer_enqueued_row_count[layer_index] == 0u )
				queue->layer_sealed[layer_index] = 0u;
